...
public:
    ...
    int getProgram(cl_program *program, cl_context context, const char *kernelSourceCode, string compile_flags);
    int getProgramFromFile(cl_program *program, cl_context context, const char *sourceFilename, string compile_flags);
...
int BasicCL::getProgram(cl_program *program, cl_context context, const char *kernelSourceCode, string compile_flags)
{
    ...
    compile_flags += string(" -cl-std=CL2.0");

	_ciErr = clBuildProgram(program[0], 0, NULL, compile_flags.c_str(), NULL, NULL);
    if(_ciErr != CL_SUCCESS) return _ciErr;
    return CL_SUCCESS;
}
...
int BasicCL::getProgramFromFile(cl_program *program, cl_context context, const char *sourceFilename, string compile_flags)
{
    char *kernelSourceCode;
    kernelSourceCode = readSource(sourceFilename);
    int err = getProgram(program, context, kernelSourceCode, compile_flags);
    return err;
}
