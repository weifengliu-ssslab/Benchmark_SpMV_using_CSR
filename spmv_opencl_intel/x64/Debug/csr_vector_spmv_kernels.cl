                       ...
                       const unsigned int THREADS_PER_VECTOR)
{
    ...
    const int thread_lane = local_id % THREADS_PER_VECTOR; //local_id & (THREADS_PER_VECTOR - 1);          // thread index within the vector
    ...
    for(int row = vector_id; row < num_rows; row += num_vectors)
    {
        ...
        if (THREADS_PER_VECTOR == 8 && row_end - row_start > 8)
        {
