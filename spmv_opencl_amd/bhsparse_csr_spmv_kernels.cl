...
inline
void sum_32(volatile __local vT *s_sum,
            const int            local_id)
{
...
inline
void sum_64(volatile __local vT *s_sum,
            const int            local_id)
{
...
inline
void scan_32(volatile __local unsigned int *s_scan,
             const int                      local_id)
{
    int ai, bi;
    ...
    if (!local_id)      { s_scan[31] = s_scan[15]; s_scan[15] = 0; }
...
inline
void scan_64(volatile __local unsigned int *s_scan,
             const int                      local_id)
{
    int ai, bi;
    int baseai = 1 + 2 * local_id;
    ...
    if (!local_id)     { s_scan[63] = s_scan[31]; s_scan[31] = 0; }
...
void SpMV_kernel(__global const int             *d_csrRowPtrA,
                 ...
                 __local vT                     *s_interres, //s_interres[][SEG_H * (THREADBUNCH+1)],
                 ...
                 const int                       m)
{
    ...
    volatile __local vT *s_interres_local = &s_interres[tbunch_lid * SEG_H * (THREADBUNCH + 1)];
    ...
    for (int i = 0; i < STEP; i++)
    {
...
#if THREADBUNCH == 32
            sum_32(s_interres_local, tbunch_tid);
#elif THREADBUNCH == 64
            sum_64(s_interres_local, tbunch_tid);
#endif
            if (!tbunch_tid)
            {
        ...
        for (int j = 0; j < SEG_H; j++)
        {
            ...
            // load indices & value, and keep index in the right scope [0,nnz)
            s_interres_local[y * (THREADBUNCH + 1) + x] = candidate_idx < nnz ? d_csrValA[candidate_idx] * d_x[d_csrColIndA[candidate_idx]] : 0;
        }
...
#if THREADBUNCH == 32
        scan_32(&s_ibuff[tbunch_lid * (THREADBUNCH+1)], tbunch_tid);
#elif THREADBUNCH == 64
        scan_64(&s_ibuff[tbunch_lid * (THREADBUNCH+1)], tbunch_tid);
#endif
